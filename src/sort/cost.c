...
static int	get_do_r_cost(t_stack **stack, int value)
{
	...
	index = ft_stackindex(*stack, value);
	if (index == -1)
		return (-1);
	return (index);
}
					...
					, t_stack **stack_b, t_stack *target)
{
	...
	{
		total_a = get_do_r_cost(stack_a, source->value);
		total_b = get_do_r_cost(stack_b, target->value);
		if (!get_rotation_way(source, stack_a))
		{
			total_a = get_do_rr_cost(stack_a, source->value);
			total_b = get_do_rr_cost(stack_b, target->value);
		}
		if (total_a > total_b)
			return (total_a);
		return (total_b);
